...
public class TestimonialController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Testimonial was modifed",
                    content = { @Content(mediaType = "application/json",
                            ...
                            examples = {@ExampleObject(name = "Testimonial was update", summary = "Testimonial update", description = "when the required fields are completed, the user is updated and the entity update is sent.", value = MODEL_TESTIMONIAL
                            )})}),
            ...
            @ApiResponse(responseCode = "404", description = "Testimonial not found",
                    content = { @Content(mediaType = "application/json",
                            schema = @Schema(implementation = MessageInfo.class),
                            examples = {@ExampleObject(name = "Example error 404", summary = "Testimonial not found", description = "When you enter the id of a nonexistent testimonial in the route, it sends a message error 400 (Not Found).", value = MODEL_TESTIMONIAL_ERROR_404
                            )}) })})
    ...
    @ApiResponse(responseCode = "200", description = "Testimonial was deleted",
            content = { @Content(mediaType = "application/json",
                    schema = @Schema(implementation = MessageInfo.class),
                    examples = {@ExampleObject(name = "Testimonial deleted", summary = "Testimonial deleted", description = "The testimonial is successfully removed when found by id, then a success message is sent.", value = MODEL_TESTIMONIAL_DELETE
                    )}) })
    @ApiResponse(responseCode = "400", description = "Error for the character entered",
            content = { @Content(mediaType = "application/json",
                    schema = @Schema(implementation = MessageInfo.class),
                    examples = {@ExampleObject(name = "Example error 400", summary = "Error character", description = "When I enter an invalid character in the path it sends an error message 400 (Bad Request).", value = MODEL_TESTIMONIAL_ERROR_400
                    )}) })
    @ApiResponse(responseCode = "404", description = "Testimonial not found",
            content = { @Content(mediaType = "application/json",
                    schema = @Schema(implementation = MessageInfo.class),
